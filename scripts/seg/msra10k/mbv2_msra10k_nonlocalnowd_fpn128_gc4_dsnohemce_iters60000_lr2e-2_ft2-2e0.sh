...
CHECKPOINTS_NAME="mbv2_msra10k_nonlocalnowd_fpn128_gc4_dsnohemce_iters60000_lr2e-2"$2
PRETRAINED_MODEL=$4
...
GC_BLOCK="--gc_index 13 --gc_ratio 2"
...
if [[ "$1"x == "train"x ]]; then
  ...
  ${DIST_PYTHON} main.py --config_file ${CONFIG_FILE} --phase train --train_batch_size 2 --val_batch_size 1 \
                         ...
                         --resume_continue y --resume_val n --resume ./checkpoints/seg/msra10k/${CHECKPOINTS_NAME}_latest.pth \
                         --extra_msrab_ratio 2.0 \
