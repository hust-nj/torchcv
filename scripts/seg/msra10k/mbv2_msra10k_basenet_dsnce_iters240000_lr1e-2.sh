...
MODEL_NAME="basenet"
CHECKPOINTS_NAME="mbv2_msra10k_basenet_dsnce_iters240000_lr1e-2"$2
PRETRAINED_MODEL=$4
...
NGPUS=4
DIST_PYTHON="${PYTHON} -m torch.distributed.launch --nproc_per_node=${NGPUS} --master_port=9901"
...
if [[ "$1"x == "train"x ]]; then
  ...
  ${DIST_PYTHON} main.py --config_file ${CONFIG_FILE} --phase train --train_batch_size 2 --val_batch_size 1 \
                         ...
                         --resume_continue y --resume ./checkpoints/seg/msra10k/${CHECKPOINTS_NAME}_latest.pth \
