...
DATA_DIR=data/coco17.zip@

BACKBONE="mobilenetv2_dilated8"
MODEL_NAME="nonlocalbn"
CHECKPOINTS_NAME="fs_nonlocal_coco17_seg"$2
PRETRAINED_MODEL="checkpoints/mobilenet_v2.pth"

CONFIG_FILE='configs/seg/coco17/NL_fcn_coco17_seg.conf'
MAX_ITERS=60000
LOSS_TYPE="dsnohemce_loss"

LOG_DIR="./log/seg/coco17/"
...
if [[ "$1"x == "train"x ]]; then
  ...
  ${DIST_PYTHON} main.py --config_file ${CONFIG_FILE} --phase train --train_batch_size 2 --val_batch_size 1 \
                         --backbone ${BACKBONE} --model_name ${MODEL_NAME} --gpu 0 1 2 3 --drop_last y --syncbn y --dist y \
                         --data_dir ${DATA_DIR} --loss_type ${LOSS_TYPE} --max_iters ${MAX_ITERS} \
                         --resume_continue y --resume ./checkpoints/seg/coco17/${CHECKPOINTS_NAME}_latest.pth \
  ...
  ${PYTHON} main.py --config_file ${CONFIG_FILE} --phase test --gpu 0 1 2 3 --gather n \
                    ...
                    --resume ./checkpoints/seg/coco17/${CHECKPOINTS_NAME}_latest.pth --resume_strict y\
                    --test_dir ${DATA_DIR}/val/image --out_dir val  2>&1 | tee -a ${LOG_FILE}
  cd metric/seg/
  ${PYTHON} seg_evaluator.py --config_file "../../"${CONFIG_FILE} \
                             --pred_dir ../../results/seg/coco17/${CHECKPOINTS_NAME}/val/label \
                             --gt_dir ${DATA_DIR}/val/label  2>&1 | tee -a "../../"${LOG_FILE}

elif [[ "$1"x == "test"x ]]; then
  ${PYTHON} main.py --config_file ${CONFIG_FILE} --phase test --gpu 0 1 2 3 --gather n \
                    --backbone ${BACKBONE} --model_name ${MODEL_NAME} --checkpoints_name ${CHECKPOINTS_NAME} \
                    --resume ./checkpoints/seg/coco17/${CHECKPOINTS_NAME}_latest.pth \
